...
enum ImGuiFileBrowserFlags_ {
	...
	ImGuiFileBrowserFlags_NoMove = 1 << 8,
};
	...
	class FileBrowser {
	  public:
		...
		void SetWindowSize(int width, int height) noexcept,
			SetWindowPosition(const ImVec2 &position) noexcept;

		ImVec2 GetWindowSize(), GetWindowPosition();
...
inline void ImGui::FileBrowser::SetWindowPosition(
	const ImVec2 &position) noexcept {
...
inline ImVec2 ImGui::FileBrowser::GetWindowPosition() { return position; }
...
inline void ImGui::FileBrowser::Display() {
	...
	if (!position_) {
		if (position.x > -1 && position.y == -1)
			SetNextWindowPos({position.x, ImGui::GetWindowPos().y});
		if (position.y > -1 && position.x == -1)
			SetNextWindowPos({ImGui::GetWindowPos().x, position.y});
		if (position.x > -1 && position.y > -1) SetNextWindowPos(position);
	...
	if (flags_ & ImGuiFileBrowserFlags_NoModal) {
		if (!BeginPopup(openLabel_.c_str())) return;
								...
								(flags_ & ImGuiFileBrowserFlags_NoTitleBar
									 ...
									 : 0)
									| (flags_ & ImGuiFileBrowserFlags_NoMove
